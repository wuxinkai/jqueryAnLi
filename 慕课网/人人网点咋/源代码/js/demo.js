...
window.onload = function () {
    ...
    function praiseBox(box, el) {
        ...
        var oldTotal = parseInt(praisesTotal.getAttribute('total')); //获取total内容  ，parseInt专成数字
        var newTotal;  //变量不知道加还是减
        if (txt == '赞') { //就往上加
            newTotal = oldTotal + 1;
            ...
            newTotal = oldTotal - 1;
            praisesTotal.setAttribute('total', newTotal); //把点赞之后的总数再付给 total上
    ...
    for (var i = 0; i < boxs.length; i++) {
        ...
        boxs[i].onclick = function (e) {
            ...
            switch (el.className) {
                ...
                //关闭分享
                case 'close': //循环如果className ==close 就删除他的父亲
                    removeNode(el.parentNode);  //22行的方法
                    break;

                //赞分享
                case 'praise':
                    praiseBox(el.parentNode.parentNode.parentNode, el); //取三次父节点，就找到了li ，el是要触发的元素
                    break;
                ...
                //回复按钮灰  如果点击的是btn-off这个按钮，就让他清楚定时器
                case 'btn btn-off':
                    clearTimeout(timer);
                    break;
